...
Nowadays, we have better ways to get secret messages to people. Image Steganography using the LSB method is the most common, and there are a few tools that can help with decoding / decrypting steg like this. Lets run through a few of them. Most of them do the same thing, so find one that you like and figure out how to use that one.

### [StegOnline](https://georgeom.net/StegOnline/upload)
Steg Online is one of the fastest tools around for image steg. My favorite is the LSB half button. More on this later.
